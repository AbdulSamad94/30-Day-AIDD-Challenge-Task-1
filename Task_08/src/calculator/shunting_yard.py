    ...
    def convert_to_rpn(self, tokens):
        ...
        for token_type, token_value in tokens:
            if token_type == "NUMBER":
                ...
                while (
                    operator_stack
                    and operator_stack[-1][0] == "OPERATOR"
                    and (
                        (
                            self.operator_associativity[token_value] == "left"
                            and self.operator_precedence[token_value]
                            <= self.operator_precedence[operator_stack[-1][1]]
                        )
                        or (
                            self.operator_associativity[token_value] == "right"
                            and self.operator_precedence[token_value]
                            < self.operator_precedence[operator_stack[-1][1]]
                        )
                    )
                ...
                if not operator_stack:
                    raise ValueError(
                        "Mismatched parentheses: No matching left parenthesis"
                    )
        ...
        while operator_stack:
            if operator_stack[-1][0] == "LPAREN":
                raise ValueError(
                    "Mismatched parentheses: No matching right parenthesis"
                )
            output_queue.append(operator_stack.pop()[1])
